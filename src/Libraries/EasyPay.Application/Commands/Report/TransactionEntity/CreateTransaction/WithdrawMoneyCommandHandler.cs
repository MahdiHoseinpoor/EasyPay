...
{
    public class WithdrawMoneyCommandHandler : IRequestHandler<WithdrawMoneyCommand, Result<Guid>>
    {
        private readonly IAccountRepository _accountRepository;
        private readonly ITransactionRepository _transactionRepository;
        private readonly ICurrentUserService _currentUserService;

        public WithdrawMoneyCommandHandler(IAccountRepository accountRepository, ITransactionRepository transactionRepository, ICurrentUserService currentUserService)
        {
        ...
        public async Task<Result<Guid>> Handle(WithdrawMoneyCommand request, CancellationToken cancellationToken)
        {
            ...
            if (account.CurrentBalance < request.Amount)
                return Result<Guid>.Failure(new Error(400, "Insufficient funds for this withdrawal."));
