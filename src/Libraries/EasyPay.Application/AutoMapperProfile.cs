...
namespace EasyPay.Application
{
    ...
    .ForMember(dest => dest.AccountNumber, opt => opt.Ignore()); 
            ...
            CreateMap<AccountTypeDocumentRequirement, AccountTypeDocumentRequirementDto>()
              .ForMember(dest => dest.AuthItemTitle, opt => opt.MapFrom(src => src.AuthItem != null ? src.AuthItem.Title : "Unknown"));
            CreateMap<AuthItemValue, PendingSubmissionDto>()
                    .ForMember(dest => dest.AuthItemValueId, opt => opt.MapFrom(src => src.Id))
                    .ForMember(dest => dest.UserFullName, opt => opt.MapFrom(src => src.User.FullName))
                    .ForMember(dest => dest.UserPhoneNumber, opt => opt.MapFrom(src => src.User.PhoneNumber))
                    .ForMember(dest => dest.AuthItemTitle, opt => opt.MapFrom(src => src.AuthItem.Title));

            CreateMap<AuthItemValue, SubmissionDetailDto>()
                .ForMember(dest => dest.AuthItemTitle, opt => opt.MapFrom(src => src.AuthItem.Title))
                .ForMember(dest => dest.AuthItemDescription, opt => opt.MapFrom(src => src.AuthItem.Description))
                .ForMember(dest => dest.AuthItemType, opt => opt.MapFrom(src => src.AuthItem.AuthItemType))
                .ForMember(dest => dest.UserFullName, opt => opt.MapFrom(src => src.User.FullName))
                .ForMember(dest => dest.UserPhoneNumber, opt => opt.MapFrom(src => src.User.PhoneNumber))
